...
let add_literal_to_list (lit : literal) ( lits : list literal) : (res : list literal{
    (forall (l : literal{l = lit || L.contains l lits}). (L.contains l res)) /\ (length res = length lits + 1)
}) = lit :: lits
...
let rec remove_literal_from_list (lits : list literal) (lit : literal{L.contains lit lits}) 
    ...
    = let x = L.hd lits in
        if x = lit 
        then L.tl lits
        else add_literal_to_list x (remove_literal_from_list (L.tl lits) lit)
...
let rec remove_non_pure_literals (all_lits : list literal) 
    ...
    =  if length all_lits = 0 then [] else
        ...
        then remove_non_pure_literals (remove_literal_from_list xs negated_x )
