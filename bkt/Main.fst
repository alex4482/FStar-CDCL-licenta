...
let rec lemma_test_17 (t : truth_assignment) (c : clause) : Lemma
    ...
    = 
        ...
        //                 {forall (v : variable_info{List.Tot.contains v t}). (List.Tot.contains v other_t)}).
        //             (are_clause_vars_in_assignment other_t c)));
        
        // assert((forall (other_t : truth_assignment
        //                 {forall (v : variable_info{List.Tot.contains v t}). (List.Tot.contains v other_t)}).
        //             (forall (l : literal{List.Tot.contains l c}). (get_literal_value t l = get_literal_value other_t l))));
...
// let lemma_test_16 (t : truth_assignment) ( c : clause) : Lemma
//     (requires are_clause_vars_in_assignment t c)
//     (ensures (forall (other_t : truth_assignment { forall (v : variable_info{List.Tot.contains v t}). (List.Tot.contains v other_t)}). 
//         (
//             (forall (l : literal{List.Tot.contains l c}). (get_literal_value t l = get_literal_value other_t l))
//             /\ List.Tot.contains true (get_values_from_clause t c) = List.Tot.contains true (get_values_from_clause other_t c))))
//     = 
//         assert(forall (other_t : truth_assignment { forall (v : variable_info{List.Tot.contains v t}). (List.Tot.contains v other_t)}). 
//         (are_clause_vars_in_assignment other_t c));
//         assert(forall (other_t : truth_assignment 
//                 { forall (v : variable_info{List.Tot.contains v t}). (List.Tot.contains v other_t)}).
//                     (forall (l : literal{List.Tot.contains l c}). (get_literal_value t l = get_literal_value other_t l)));
...
let rec sat_bkt'    (f: formula { length f > 0 }) 
    ...
    then if are_variables_in_truth_assignment f t
        then 
            ...
            let newVariable = length t + 1 in
            let new_var_info_false = {value=false; variable=newVariable} in
            lemma_test_9 (get_vars_in_formula f) t;
            assume(is_variable_in_assignment t new_var_info_false.variable = false);
            assume(List.Tot.contains new_var_info_false.variable (get_vars_in_formula f));
            ...
            let newT = add_var_to_truth t new_var_info_false in
                ...
                if NotSat? res_1 = true  
                    then 
                    //de ajutat cu un assert care spune ca nu se poate extinde cu 0
                        ...
                        assert(( forall (whole_t: truth_assignment{ 
                                    ...
                                    /\ length whole_t =  length ( get_vars_in_formula f)}). 
                                is_solution f whole_t = false));
                        assume(count_variables_occurrence t new_var_info_true.variable = 0);
                        assume(List.Tot.contains new_var_info_true.variable (get_vars_in_formula f));
                        let newT2 = add_var_to_truth t new_var_info_true in
                            ...
                            res_2
                            //de ajutat daca e fals cu un assert, care  spune ca nu se poate extinde nici cu 0 nici cu 1
                    else res_1
    else 
    let ress = NotSat in
